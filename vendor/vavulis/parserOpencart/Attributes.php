...
//    {
//        $buf_atr = []; // очередь на создание в базе данных атрибутов (массив объектов Attribute с недозаполненными свойствами класса)
//        $buf_group_and_atr = []; // очередь на создание в базе данных групп атрибутов (массив объектов Attribute с недозаполненными свойствами класса)
//        // наполняем очереди
//        foreach ($attributes as $a) {
//            $same_group_name = 0;
//            $same_group_and_name = 0;
//            foreach ($this->attributes as $tt) {
//                if ($a['group'] == $tt->group_name) {
//                    if ($a['name'] == $tt->name) {
//                        // такой атрибут уже есть, то есть и группа и атрибут существуют в $this->attributes
//                        $same_group_and_name = 1;
//                        $this->attributes[$tt->id]->to_product = 1; // создавать атрибуты и группы уже не надо. только привязать к товару потом
//                        $this->attributes[$tt->id]->val = $a['val']; // задаем значение атрибута
//                    } else {
//                        // равны только группы
//                        $same_group_name = 1;
//                        $t = new Attribute($a['group'], $tt->group_id, $a['name'], null, 0, 1, 0, $a['val']);
//                        $buf_atr[] = $t; // буфер для создания атрибутов
//                    }
//                }
//            }
//            if (!$same_group_and_name && !$same_group_name) {
//                //группы разные
//                $t = new Attribute($a['group'], null, $a['name'], null, 0, 1, 0, $a['val']);
//                // пополняем очередь на добавление групп  и атрибутов в базу
//                $buf_group_and_atr[] = $t; // буфер для создания групп и атрибутов
//            }
//        }
//        // создаем в базе группы_атрибутов и атрибуты из buf_group_and_atr[]
//        foreach ($buf_group_and_atr as $tt) {
//            $new_group_id = $this->createGroupOfAttributes($dbh, $tt->group_name); // в базе создана новая категория. вернули ее id
//            $new_atr_id = $this->createAttribute($dbh, $new_group_id, $tt->name); // в базе создан новый атрибут с заданной категорией. вернули его id
//            // заполняем недостающие параметры объекта Атрибут
//            $tt->group_id = $new_group_id;
//            $tt->id = $new_atr_id;
//            $tt->exists_in_db = 1; // и группа_атрибута, и сам атрибут уже есть в базе
//            // добавляем новый атрибут
//            $this->addAttribute($tt);
//        }
//
//        // создаем в базе атрибуты из $buf_atr[]
//        foreach ($buf_atr as $tt) {
//            $new_atr_id = $this->createAttribute($dbh, $tt->group_name, $tt->name); // в базе создан новый атрибут с заданной категорией. вернули его id
//            $tt->id = $new_atr_id;
//            $tt->exists_in_db = 1; // и группа_атрибута и сам атрибут уже есть в базе
//            // добавляем новый атрибут
//            $this->addAttribute($tt);
//        }
//
//        // назначаем атрибуты товару
//        foreach ($this->attributes as $tt) {
//            if ($tt->to_product == 1) {
//                $this->setAttributeToProduct($dbh, $product_id, $tt);
//            }
//            // больше этот атрибут к товару назначать не надо. укажем это
//            $this->attributes[$tt->id]->to_product = 0;
//            $this->attributes[$tt->id]->added_to_product = 1;
//        }
//
//    }
//    function findSimilar($group_name, $name)
//    {
//        findA...
//        findB...
//        findC...
